...
function Nav() {
    ...
    useEffect(()=>{
        if(input===""){
            ...
            let filtered=food_items.filter((item)=>item.food_category.includes(input)||item.food_name.toLowerCase().includes(input.toLowerCase()))
            setCate(filtered)
        }
    },[input])
    ...
    return (
        <div className="w-full h-25 flex justify-between items-center px-5 md:px-8" >
           ...
           <form className="w-[45%] h-15 bg-white flex  items-center px-5 gap-5 rounded-md shadow-md md:w-[70%]"onSubmit={(e)=>e.preventDefault()}>
